...
public class RegistrationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("Registration attempt: username=" + username + ", email=" + email + ", phone=" + phone + ", city=" + city + ", address=" + address);
        ...
        try {
            ...
            account.setPassword(password); // Sẽ được mã hóa MD5 trong AccountDAO
            account.setEmail(email.trim().toLowerCase());
            account.setPhone(phone);
            account.setRole(0); // Khách hàng
            ...
            try {
                ...
                System.out.println("Authentication failed for email to " + email + ": " + e.getMessage());
                request.setAttribute("error", "Lỗi gửi email OTP: Thông tin xác thực email không đúng.");
                request.getRequestDispatcher("/dangky/register.jsp").forward(request, response);
                return;
                ...
                System.out.println("Email configuration error: " + e.getMessage());
                request.setAttribute("error", "Lỗi cấu hình email. Vui lòng thử lại sau hoặc liên hệ quản trị viên.");
                request.getRequestDispatcher("/dangky/register.jsp").forward(request, response);
                return;
            }
            ...
            System.out.println("Adding token: CustomerID=" + customerId + ", Token=" + otp + ", Time_Add=" + timeAdd + ", Time_Exp=" + token.getTimeExp());

            TokenDAO tokenDAO = new TokenDAO();
            tokenDAO.insertToken(token);
            System.out.println("Token saved for CustomerID=" + customerId + ", Token=" + otp);
